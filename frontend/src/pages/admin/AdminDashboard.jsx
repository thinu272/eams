...
import { BuildingOffice2Icon, CreditCardIcon, ShieldCheckIcon, TicketIcon, UsersIcon, CheckBadgeIcon, CurrencyDollarIcon, BanknotesIcon, CheckCircleIcon, XCircleIcon, ArrowDownTrayIcon, EyeIcon, FunnelIcon } from '@heroicons/react/24/outline';
...
const SectionFilters = ({ section, params, updateQuery, organiserOptions = [] }) => {
  if (section === 'overview' || section === 'settings' || section === 'bank-accounts' || section === 'payments') return null;
  return (
...
const PaymentsSection = () => {
  ...
  const [eventFilter, setEventFilter] = useState(params.get('eventId') || '');
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      const response = await getAllPayments(queryParams);
  ...
  useEffect(() => {
    ...
    fetchStatistics();
    fetchEvents();
  }, [params.toString(), eventFilter, statusFilter]);
      ...
      <Card className="rounded-[28px] border-slate-200" padding={false}>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            <CreditCardIcon className="mx-auto h-12 w-12 text-slate-300" />
                ...
                {payments.map((payment) => {
                  ...
                  return (
                    <Tr key={payment._id}>
                      ...
                      <Td>
                        <div>
                          <p className="text-sm font-medium text-slate-900">{payment.buyer?.name || payment.payerName || '-'}</p>
                          <p className="text-xs text-slate-500">{payment.buyer?.email || payment.payerEmail || '-'}</p>
                        </div>
                      </Td>
                      <Td>
                        <div>
