...
import { getAllEventsAdmin, createEvent, publishEvent, updateEvent } from '../../api/events';
import api from '../../api/client'; // For direct delete call if helper missing or import it
...
const AdminEvents = () => {
  ...
  const [assigning, setAssigning] = useState(null); // ID of event being assigned
  const [form, setForm] = useState({ name: '', 'venue.name': '', 'venue.city': '', 'venue.country': 'Sri Lanka', startDate: '', endDate: '', description: '', mainOrganiser: '' });

  const load = () => getAllEventsAdmin({ limit: 50 }).then(r => setEvents(r.data.data.events)).finally(() => setLoading(false));
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (form.mainOrganiser) payload.mainOrganiser = form.mainOrganiser;
      
      await createEvent(payload);
      toast.success('Event created');
      setShowCreate(false);
      setForm({ name: '', 'venue.name': '', 'venue.city': '', 'venue.country': 'Sri Lanka', startDate: '', endDate: '', description: '', mainOrganiser: '' });
      load();
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-gray-900">Events</h1><p className="text-gray-500 text-sm">Manage all events</p></div>
        <Button onClick={() => setShowCreate(true)}>+ New Event</Button>
      </div>

      <Table>
        <thead><tr><Th>Event</Th><Th>Date</Th><Th>Venue</Th><Th>Organiser</Th><Th>Status</Th><Th>Actions</Th></tr></thead>
        <tbody>
          {events.map(event => (
            <Tr key={event._id}>
              ...
              <Td>
                <div className="flex gap-2">
                  <Link to={`/admin/events/${event._id}`} className="text-xs text-blue-600 hover:underline">Edit</Link>
                  {event.status === 'draft' && <button onClick={() => handlePublish(event._id)} className="text-xs text-green-600 hover:underline">Publish</button>}
                  <button onClick={() => handleDelete(event._id)} className="text-xs text-red-600 hover:underline">Delete</button>
                </div>
              </Td>
            </Tr>
          ))}
