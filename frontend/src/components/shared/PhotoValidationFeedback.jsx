...
const PhotoValidationFeedback = ({
  ...
  if (validating) {
    return (
      <p className="mt-2 text-xs font-medium text-blue-600">Analyzing photo quality and face detection...</p>
    );
  }
          ...
          {(faceAnalysis?.faceCount > 0 || qualityAnalysis.resolution) && (
            <div className="mt-2 grid gap-1 text-xs text-slate-600 sm:grid-cols-2">
              {qualityAnalysis.resolution && (
                <div>Resolution: {qualityAnalysis.resolution.dimensions?.width}x{qualityAnalysis.resolution.dimensions?.height}</div>
              )}
              {faceAnalysis && <div>Face count: {faceAnalysis.faceCount}</div>}
              {faceAnalysis?.confidence > 0 && (
                <div>Face confidence: {(faceAnalysis.confidence * 100).toFixed(1)}%</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3">
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-amber-900">Photo validation issues</p>
